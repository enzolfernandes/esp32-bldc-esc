...
Para garantir um torque constante, a frequência de chaveamento deve ser suficientemente alta para interagir com a indutância do motor, limitando a ondulação de corrente (\textit{ripple}). A frequência de $20 kHz$ oferece um período de chaveamento ($T = 50 \mu s$) compatível com a constante de tempo elétrica ($\tau = L/R$) do motor selecionado, preservando a densidade de torque em baixas rotações sem incorrer nas penalidades de eficiência de frequências mais altas \cite{hanselman2003brushless}.
%TODO: Aferir Constante do motor.
