...
\input{capitulos/1_objetivos.tex}
...
\section{Controlador Eletrônico de Velocidade (ESC)} 
 ...
 Esse comportamento ocorre porque os enrolamentos do estator são indutivos. A corrente de um elemento indutivo não pode variar instantaneamente, logo, quando um MOSFET é bloqueado, a corrente que circulava naquela fase não desaparece. Ela força a tensão do terminal da fase a se deslocar até polarizar diretamente o diodo de corpo de algum MOSFET. A corrente que estava circulando pela fase de saída continua fluindo pelo enrolamento da fase de saída e é desviada para um caminho de roda livre formado pelos diodos intrínsecos dos MOSFETs, até que sua energia magnética seja transferida para a outra fase.

 Considere, por exemplo, a transição do setor $A^+B^-$ para o setor $A^+C^-$. Antes da comutação, a corrente principal percorre o caminho $+V_{DC} \rightarrow H_A \rightarrow A \rightarrow B \rightarrow L_B \rightarrow -V_{DC}$, onde $H_A$ representa o transistor superior da fase A e $L_B$ o transistor inferior da fase B. Quando $L_B$ é desligado, a corrente que saía pela fase B continua no mesmo sentido por causa da energia armazenada no indutor. Como o caminho pelo transistor inferior foi aberto, o terminal B é elevado até conduzir pelo diodo de corpo do transistor superior da mesma fase ($H_B$), fechando uma malha de recirculação. Ao mesmo tempo, $L_C$ começa a conduzir e a corrente cresce na nova fase de retorno C. Durante esse intervalo, a corrente se divide entre uma malha antiga em decaimento, associada à fase B, e uma malha nova em crescimento, associada à fase C.

O caso complementar ocorre quando a fase que está saindo é conectada ao terminal positivo do barramento CC. Em uma transição como $A^+C^-$ para $B^+C^-$, o transistor superior de A é bloqueado e a corrente da fase A, que ainda tende a entrar no enrolamento, força o terminal A a conduzir pelo diodo de corpo do transistor inferior de A ($L_A$). Assim, quando um transistor superior é bloqueado, o caminho complementar é formado pelo diodo em antiparalelo do transistor inferior da mesma fase e quando um transistor inferior é bloqueado, o caminho complementar é formado pelo diodo em antiparalelo do transistor superior da mesma fase. A Tabela \ref{tab:subintervalos_comutacao_6step} resume essa sequência para uma rotação elétrica completa.
