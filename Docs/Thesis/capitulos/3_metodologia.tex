...
O \textit{payload} JSON transmitido consolida, em chaves compactas, o estado da FSM, correntes de fase, tensão de barramento, RPM, parâmetros do PI, latência do \texttt{motor\_control\_tick()}, \textit{heap} livre, status do controle PS4 (\texttt{ps4c}, R2, Circle) e código de falha ativo. A telemetria serial por UART ($115200\,\text{baud}$) permanece disponível para depuração em laboratório, porém os ensaios de potência com motor em rotação foram conduzidos preferencialmente sem cabo USB conectado, utilizando exclusivamente a dashboard Wi-Fi como canal de monitoramento.
...
O ESP32 suporta múltiplos ambientes de desenvolvimento e linguagens de programação. Entre as opções disponíveis, três se destacam na comunidade de sistemas embarcados: \textbf{MicroPython}, \textbf{C puro} e \textbf{C/C++} via framework Arduino com acesso direto ao ESP-IDF.
...
A técnica de detecção de cruzamento por zero da FCEM possui uma limitação física intrínseca: a amplitude da FCEM é proporcional à velocidade do rotor ($e = K_e \cdot \omega$). Na partida ($t=0$), a FCEM é nula, impossibilitando a determinação da posição do rotor pelos comparadores. Para contornar isso, projeta-se uma máquina de estados finitos composta por três estágios, conforme ilustrado na Figura~\ref{fig:fsm_estados}:
...
% --- Estados principais (linha horizontal) ---
...
% --- Estado de falha (abaixo do centro) ---
...
% --- Recuperacao de FAULT ---
\path[->]
    (FAULT) edge[bend right=10] node[lbl, below left] {\textit{clear fault} (Options)} (IDLE);
\end{tikzpicture}
}%
\caption{M\'aquina de estados finita do ESC: estados operacionais e transi\c{c}\~oes de falha.}
\caption*{Fonte: Elaborado pelo autor.}
...
\begin{enumerate}
    ...
    Assim que o microcontrolador detecta uma sequência válida de cruzamentos por zero (ZCD) nos comparadores, o sistema realiza o \textit{handover}. O temporizador de malha aberta é desativado e o controle passa a ser orientado a eventos: cada detecção de ZCD dispara uma rotina de interrupção que, após um atraso calculado de $30^\circ$ elétricos, executa a próxima comutação de fase. A partir deste momento, o motor opera em malha fechada, auto-sincronizado.
\end{enumerate}
