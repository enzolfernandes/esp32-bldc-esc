...
Este ensaio teria como objetivo registrar, com pinças de corrente nos três terminais de fase, a evolução temporal das correntes de estator durante o ciclo completo de partida, comandado pelo gatilho R2 do DualShock\,4 ou pela interface serial (\texttt{A} + setpoint). Verificar-se-ia a integridade da sequência \texttt{ALIGN} $\to$ \texttt{RUN\_OPEN} $\to$ \texttt{RUN\_SPEED} no domínio elétrico (formas trapezoidais de corrente) e temporal (duração de cada estado).
...
Em \texttt{RUN\_OPEN}, a frequência elétrica incrementar-se-ia linearmente até $300\,\text{Hz}$; as correntes de fase apresentariam o padrão trapezoidal característico da comutação de seis passos (pulsos de $120°$ elétricos, intervalo Hi-Z de $60°$). A transição para \texttt{RUN\_SPEED} ocorreria quando o RPM estimado superasse 600\,RPM por 200\,ms consecutivos.
