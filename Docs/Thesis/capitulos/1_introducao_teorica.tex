...
\begin{equation}
    K_T \text{ [N·m/A]} = \frac{2\pi }{60 \cdot K_V \text{ [RPM/V]}}
\end{equation}
...
\section{Controlador Eletrônico de Velocidade (ESC)} 
 ...
 Embora o modelo idealizado assuma a condução de apenas dois transistores simultaneamente, a natureza indutiva dos enrolamentos do estator impede que a corrente se extinga instantaneamente durante as transições de estado. Na prática, existe um intervalo de comutação transitório (overlap temporal) onde a corrente da fase que está sendo bloqueada continua fluindo através do seu diodo de roda livre intrínseco para o barramento de alimentação. Concomitantemente, o transistor da nova fase a ser magnetizada entra em condução. Durante esta fração de tempo, o circuito equivalente intermediário conta com três semicondutores conduzindo simultaneamente. 
 
 Vale ressaltar que esses eventos de condução e bloqueio, e a consequente distorção transiente da tensão de fase gerada por esse terceiro caminho de corrente, são as principais causas das oscilações no torque do motor (\textit{torque ripple}) características deste método \cite{kim2017electric}.
